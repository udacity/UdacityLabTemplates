...
try
{
  ...
  $labResourceGroupName = "UDACITY-CYBERND02-EXERCISE2"
  ...
  $udacityImageDefinitionName = "CYBERND02-Student-StudentVM5"
  ...
  $myNetworkSecurityGroupRuleRDP = "sshnsgrule"
  $nsgRuleRDP = New-AzNetworkSecurityRuleConfig -Name $myNetworkSecurityGroupRuleRDP  -Protocol Tcp -Direction Inbound -Priority 1010 -SourceAddressPrefix * -SourcePortRange * -DestinationAddressPrefix * -DestinationPortRange 22 -Access Allow
  ...
  New-AzVM -ResourceGroupName $labResourceGroupName -Location $labResourceGroupLocation -VM $vmConfig -Verbose -DisableBginfoExtension
